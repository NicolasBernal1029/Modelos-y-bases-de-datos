...
ADD CONSTRAINT chk_correo_INSITUCIONAL 
CHECK (correoInstitucional LIKE '%_@__%.__%' AND correoInstitucional NOT LIKE '% %');
...
--TId
ALTER TABLE Errores
ADD CONSTRAINT chk_id_errores
CHECK(id LIKE '%D__%' OR '%U__%' or '%I__%' OR '%S__%'
);

--TTipo
ALTER TABLE Evidencias
ADD CONSTRAINT chk_tipo_evidencia
CHECK(tipo LIKE 'log' OR 'archivo' or 'descripcion' OR 'técnica'
);

--TRuta
ALTER TABLE Evidencias
ADD CONSTRAINT chk_ruta_evidencia
CHECK(IF(
    rutaAsociada LIKE 'descripción técnica' AND rutaAsociada = NULL)
);
